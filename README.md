...
### 🧠 Free local LLM with Ollama ($0 cost)

Want the "real AI" mode without paying OpenAI? Run Llama 3.1 on your own machine —
QuickSave speaks Ollama's OpenAI-compatible endpoint, so setup is 3 env vars:

1. Install Ollama → [ollama.com/download](https://ollama.com/download) (Windows / Mac / Linux)
2. Download the model (one-time, ~4.7 GB, needs ~8 GB RAM):
   ```bash
...
5. The top bar flips from `✨ AI: Local mode` to `✨ AI: llama3.1` — chat + insights
   are now generated by your local model, grounded in the demo's live data.

Prefer Docker? `docker compose --profile ollama up --build`, then in another terminal
`docker compose exec ollama ollama pull llama3.1` (with `OPENAI_BASE_URL=http://ollama:11434/v1` in `.env`).

> If Ollama isn't running, the app silently falls back to local mode — the demo never breaks either way.
